...
export default function DnsPage({ onResult }) {
  ...
  const lookup = async () => {
    ...
    try {
      ...
      setResult(data);
      setHistory(prev => [{ target: trimmed, count: data.records?.length || 0, time: Date.now() }, ...prev].slice(0, 10));
      onResult?.({ type: 'dns', target: trimmed, time: Date.now() });
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="p-6 space-y-5 animate-fadeIn">
      <div>
        ...
        <p className="text-[#475569] text-xs font-mono mt-1">Resolve domain names and inspect DNS records</p>
      </div>

      <div className="card">
        <div className="flex gap-3 items-end flex-wrap">
          <div className="flex-1 min-w-48">
            <label className="text-xs font-mono text-[#475569] mb-1.5 block">Domain Name or IP</label>
            <input className="input-field" placeholder="e.g., google.com, github.com, 8.8.8.8"
              value={target} onChange={e => setTarget(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && !loading && lookup()}
              disabled={loading} />
          </div>
          <button onClick={lookup} disabled={loading || !target.trim()} className="btn-primary flex items-center gap-2">
            {loading ? <Loader size={14} className="animate-spin" /> : <Search size={14} />} Lookup
          </button>
        </div>
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                {result.reverseLookup && (
                  <div className="text-xs text-[#475569] font-mono mt-0.5">Reverse: {result.reverseLookup}</div>
                )}
              </div>
              <div className="text-right">
                <div className="font-mono text-2xl font-bold text-[#e2e8f0]">{result.records?.length ?? 0}</div>
              ...
              return (
                <div key={type} className="card">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs text-[#475569] font-mono">{TYPE_DESCRIPTIONS[type] || 'DNS Record'}</span>
                    <span className="ml-auto text-xs text-[#475569] font-mono">{records.length} record{records.length > 1 ? 's' : ''}</span>
                  </div>
                    ...
                    {records.map((rec, i) => (
                      <div key={i} className="flex items-start justify-between gap-4 p-2.5 bg-[#0a0e1a] rounded-xl border border-[#1e2d5a]">
                        ...
                        <div className="flex gap-3 shrink-0 text-xs font-mono text-[#475569]">
                          {rec.priority !== undefined && <span>Priority: {rec.priority}</span>}
                          {rec.ttl && <span><Tooltip term="TTL">TTL</Tooltip>: {rec.ttl}s</span>}
                        </div>
                      </div>
                    ))}
                  </div>

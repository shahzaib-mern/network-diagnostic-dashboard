...
const CustomDot = ({ cx, cy, payload }) => {
  if (!payload.success) return <circle cx={cx} cy={cy} r={4} fill="#ff3366" stroke="none" />;
...
export default function PingPage({ onResult }) {
  ...
  const statsRef = useRef({ rtts: [], lost: 0 });

  const startPingAsync = async () => {
    ...
    try {
      const data = await fetchPing(trimmed, count);
      ...
      const received = results.filter(r => r.success && r.rtt !== null).map(r => r.rtt);
      const lost = results.filter(r => !r.success).length;
      const total = results.length;
      const avg = received.length ? parseFloat((received.reduce((a, b) => a + b, 0) / received.length).toFixed(2)) : null;
      const summary = {
        avg,
        min: received.length ? Math.min(...received) : null,
        max: received.length ? Math.max(...received) : null,
        loss: total > 0 ? Math.round((lost / total) * 100) : 0,
        sent: total,
      ...
      setSummary(summary);
      setStatus('done');
      onResult({ type: 'ping', target: trimmed, avgRtt: summary.avg, loss: summary.loss, time: Date.now() });
    } catch (error) {
      setStatus('error');
      setErrorMsg(error.message || 'Ping failed');
    } finally {
      setRunning(false);
    }
  };
  ...
  const reset = () => {
    setResults([]); setSummary(null); setStatus('idle'); setErrorMsg('');
  };

  const validRtts = results.filter(r => r.success && r.rtt !== null);
  const avgRtt = validRtts.length
    ? Math.round(validRtts.reduce((a, b) => a + b.rtt, 0) / validRtts.length)
    : null;
    ...
    <div className="p-6 space-y-5 animate-fadeIn">
      <div>
        ...
        <p className="text-[#475569] text-xs font-mono mt-1">
          Send ICMP packets and visualize round-trip times in real time
        </p>
      </div>
      ...
      <div className="card">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex gap-2">
                  <button onClick={startPingAsync} disabled={running || !target.trim()} className="btn-primary flex items-center gap-2">
              <Play size={14} /> Start
            </button>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={results} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              ...
              <Line
                ...
                strokeWidth={2}
                dot={<CustomDot />}
                activeDot={{ r: 5, fill: '#00d4ff', stroke: '#0a0e1a' }}
                connectNulls={false}
              />
            </LineChart>
          </ResponsiveContainer>
        ...
        <div className="card border-[#00ff8822] animate-slideUp">
          ...
          {summary.loss === 0 && (
            <div className="mt-3 flex items-center gap-2 text-xs text-[#00ff88] font-mono">
              <Wifi size={12} /> Host is fully reachable with no packet loss
            </div>
          )}
          {summary.loss === 100 && (
            <div className="mt-3 flex items-center gap-2 text-xs text-[#ff3366] font-mono">
              ✕ Host is unreachable — check the address or network connectivity
            </div>
          )}
        </div>
      )}
